...
We are excited to announce the Slim 4.0.0 alpha release. General Direction, Major Changes, Changelog and Step-by-Step Hello World. Please direct all your feedback for this release to the [Slim 4 Alpha Release Feedback Thread](https://github.com/slimphp/Slim/issues/2653). The new docs are located [here](http://slim-website.lgse.com/docs/v4/).

# General Direction

For this major release the focus was on the following:
* Implementing PSR-15 Middleware Support
* Decoupling our dependency on Slim's PSR-7 implementation and enable usage of any PSR-7 implementation
* Decoupling our dependency on Pimple and enable usage of any PSR-11 ContainerInterface implementation
...
* Create an app factory that makes it easier to create an `App` instance due to the increased complexity of the increased modularity
...
* Binding of `$app` to the `RouteGroup` callable has been removed. `RouteCollectorProxy` gets injected into the `RouteGroup`'s callable instead.
